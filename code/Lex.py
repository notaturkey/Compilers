...
class Lex(object):
    ...
    def _split(self, line):
        ...
        #    print(split_line[0])
        if '.EQU' in line:
            if self._binary_re_comp.match(line.split(' ')[2]) is not None:
                return ['.EQU',  line.split(' ')[1]  ,(self._binary_re_comp.findall(line.split(' ')[2])[0])]
            elif self._hex_re_comp.match(line.split(' ')[2]) is not None:
                return ['.EQU',  line.split(' ')[1]  ,(self._hex_re_comp.findall(line.split(' ')[2])[0] )]
        else:    
            line = line.replace(' ', '')
        ...
        if len(line.split('@')) != 1:
            ...
            elif self._hex_re_comp.match(line.split('@')[1]) is not None:
